...
%thresh: Threshold for percentage of pairwise distances to consider. If left blank this will be choosen according to the smoothness of the signals. 

%Outputs:
%C: 2x1 vector of CCS estimates for the strength for the drive from x to y, C(1), and from y to x, C(2)
...
%Removes central diagonal with trivial neighboors
...
%Flattens and sorts the distance matricies
...
var_se_null=1/45*(15*rank.^2-15*rank+4);
...
if nargout==0
    ...
    for i=1:ns
        ...
        mstd_se_null(i)=sqrt(mean(var_se_null(points(i):points(i+1))))/sqrt(points(i+1)-points(i));
    end
    ...
    plot(xp,mse_null,'g')
    hold on 
    plot(xp,mse_null+mstd_se_null,'-g')
    hold on 
    plot(xp,mse_null-mstd_se_null,'-g')
    legend('xcy','ycx','null')
    xlabel('Rank')
    ...
    legend('xcy','xcy fit','ycx','ycx fit')
